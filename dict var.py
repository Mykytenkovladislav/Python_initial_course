...
def rewrite_value(x_coordinate: int, y_coordinate: int, value_input: str):
    matrix.reverse()  # Здесь и дальше matrix.reverse() для корректного отображения системы координат
    # как аналог можно 2 раза сделать в методе list_print, но я не знаю как будет правильно (жду твоего совета)
    while True:
